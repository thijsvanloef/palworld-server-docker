...
| PLAYERS<sup>1</sup>           | Max amount of players that are able to join the server                                                                                                                                              | 16             | 1-32                                                                                                       |
| PORT<sup>1</sup>              | UDP port that the server will expose                                                                                                                                                                | 8211           | 1024-65535                                                                                                 |
| PUID<sup>1</sup>              | The uid of the user the server should run as                                                                                                                                                        | 1000           | !0                                                                                                         |
| PGID<sup>1</sup>              | The gid of the group the server should run as                                                                                                                                                       | 1000           | !0                                                                                                         |
| MULTITHREADING<sup>2</sup>   | Improves performance in multi-threaded CPU environments. It is effective up to a maximum of about 4 threads, and allocating more than this number of threads does not make much sense.              | false          | true/false                                                                                                 |
...
| UPDATE_ON_BOOT<sup>2</sup>   | Update/Install the server when the docker container starts (THIS HAS TO BE ENABLED THE FIRST TIME YOU RUN THE CONTAINER)                                                                            | true           | true/false                                                                                                 |
| RCON_ENABLED<sup>3</sup>    | Enable RCON for the Palworld server                                                                                                                                                                 | true           | true/false                                                                                                 |
...
<sup>1</sup> highly recommended to set
<sup>2</sup> Make sure you know what you are doing when running this option enabled
<sup>3</sup> Required for docker stop to save and gracefully close the server
